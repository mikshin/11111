<template>
  <AppPage
    class="page-finish-test"
    :isLoading="pageIsLoading"
    :noTitle="true"
    :breadcrumbsFullPath="pageData ? pageData.breadcrumbs : null"
  >
    <div class="page-finish-test__content" v-if="pageData">
      <div class="page-finish-test__header">
        <div class="page-finish-test__test-name">{{ testName }}</div>
        <div
          class="page-finish-test__test-description"
      ...
      <TestForm
        class="page-finish-test__test-form"
        :class="{ loading: formIsLoading }"
        :key="formKey"
        :questions="pageData.data ? pageData.data : []"
        :attempts="pageData.tryingCount"
        :testResult="pageData.testResult"
        :btnResetText="'Назад к курсу'"
...
export default {
  ...
  computed: {
    testId() {
      return getLessonId(this.$route.params.testId);
    },
  ...
  methods: {
    async getDataPage() {
      try {
        ...
        const response = await axios({
          method: "get",
          url: "/mobileapp/api/learning/test/question",
          params: {
    ...
    async submit(answers) {
      try {
        ...
        const response = await axios({
          method: "post",
          url: "/mobileapp/api/learning/test/accept",
          data: {
            testId: this.testId,
            answers: this.answers,
            qAnswered: this.pageData.data,
          },
        });
        ...
        this.pageData.tryingCount = response.data.tryingCount;
        this.pageData.testResult = response.data.testResult;
      } catch (error) {
