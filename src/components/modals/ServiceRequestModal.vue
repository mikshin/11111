<template>
  ...
  >
    <div class="service_request_modal__wrapper" :class="{ loading: isLoading }">
      <div class="service_request_modal__header">
        {{ serviceModalData.serviceName }}
      </div>
      <div
        class="service_request_modal__description"
        v-html="serviceModalData.serviceDescription"
      ></div>
      <AppForm
      ...
      >
        <template #inputs>
          <InputGenerator
            v-if="!isLoading"
            :key="inputsKey"
            :inputs="serviceModalData.fields"
    ...
    <transition name="fade">
      ...
      >
        ...
        <template #error>
          <div>Внимание!</div>
          <div>
            Ошибка при отправке заявки на сервис
            {{ serviceModalData.serviceName }}!
          </div>
        </template>
      </ErrorModal>
    ...
    <transition name="fade">
      <SuccessModal
        ...
        :successText="`Заявка на сервис ${serviceModalData.serviceName} успешно отправлена`"
    ...
    async getServiceModalFields() {
      try {
        ...
        const response = await axios.get(
          `/mobileapp/api/services/getServiceAppealField?serviceId=${this.serviceID}`
        );
        if (response) {
    ...
    async submitService(i) {
      try {
        ...
        const response = await axios({
          method: "post",
          url: "/mobileapp/api/services/addServiceAppeal",
          data: i,
        });
